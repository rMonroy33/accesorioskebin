...
<div class="mb-6 flex items-center justify-between">
    <div>
        <h3 class="text-2xl font-bold text-white">Categorías</h3>
        <p class="mt-1 text-sm text-white/60">Administra las categorías principales del catálogo.</p>
    </div>
            ...
            <tbody>
                @forelse($categories as $category)
                    <tr class="border-b border-white/5">
                        ...
                        <td class="px-5 py-4 text-white/60">{{ $category->slug }}</td>
                        <td class="px-5 py-4 text-white/60">{{ $category->sort_order }}</td>
                        <td class="px-5 py-4">
                            <span class="rounded-full px-3 py-1 text-xs font-semibold {{ $category->status ? 'bg-green-500/15 text-green-300' : 'bg-red-500/15 text-red-300' }}">
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-5 py-8 text-center text-white/50">
