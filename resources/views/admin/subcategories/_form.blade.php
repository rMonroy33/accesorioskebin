<div class="grid gap-6">
        ...
        <select name="category_id"
            ...
            @foreach($categories as $category)
                <option value="{{ $category->id }}"
                    {{ (string) old('category_id', $subcategory->category_id ?? '') === (string) $category->id ? 'selected' : '' }}>
                    {{ $category->name }}
                </option>
            @endforeach
        </select>
    ...
    <div>
        ...
        <textarea name="description"
                  rows="4"
                  class="w-full rounded-2xl border border-white/10 bg-[#111111] px-4 py-3 text-white focus:border-[#c9a86a] focus:outline-none">{{ old('description', $subcategory->description ?? '') }}</textarea>
    </div>
